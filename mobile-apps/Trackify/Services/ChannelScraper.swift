...
actor ChannelScraper {
    ...
    func refreshAll() async {
        ...
        guard let container = try? ModelContainer(for: schema) else { return }
        ...
        for channel in channels {
            do {
                ...
                if let goal = channel.followerGoal, result.count >= goal {
    ...
    func scrapeFollowerCount(for channel: TrackedChannel) async throws -> (count: Int, rawText: String) {
        ...
        if channel.platform == .whatsapp {
            let regex = try NSRegularExpression(pattern: "([0-9,.]+)\\s+members", options: .caseInsensitive)
