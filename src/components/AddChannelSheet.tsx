...
const platforms = [
  { icon: "💬", label: "WhatsApp", placeholder: "https://whatsapp.com/channel/..." },
  { icon: "🎵", label: "TikTok", placeholder: "https://tiktok.com/@username" },
  { icon: "📸", label: "Instagram", placeholder: "https://instagram.com/username" },
  { icon: "▶️", label: "YouTube", placeholder: "https://youtube.com/@channel" },
];
        ...
        <div className="glass-strong rounded-t-3xl p-6 pb-10 ios-shadow">
          ...
          <div className="flex gap-2 mb-4">
            {platforms.map((p, i) => (
              <button key={i} onClick={() => setSelectedPlatform(i)}
                className={`flex-1 py-2.5 rounded-2xl text-center text-sm transition-all tap-bounce
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            <input type="url" value={url} onChange={(e) => setUrl(e.target.value)}
              ...
              className="w-full h-12 px-4 rounded-2xl glass text-foreground text-sm placeholder:text-muted-foreground outline-none focus:ring-2 focus:ring-foreground/20 transition-all disabled:opacity-50" />
